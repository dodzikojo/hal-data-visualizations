...
def create_sunburst_full_names(excel_file_path):
    ...
    # --- Apply the mapping ---
    # Replace the 'Who' column
    df['Who'] = df['Who'].replace(name_map)
    # Update the 'Created Roles' string to match
    for abbr, full_name in name_map.items():
        df['Created Roles'] = df['Created Roles'].str.replace(f'^{abbr} ', f'{full_name} ', regex=True)
    ...
    fig = px.sunburst(
        df,
        path=['Who', 'What', 'Created Roles'], # Path uses the updated columns
        values='count',
    ...
    chart_filename = os.path.join(output_dir, f'{base_filename}_sunburst_full_names.html')

    fig.write_html(chart_filename)
    print(f"Sunburst chart with full names saved as '{chart_filename}'")

if __name__ == "__main__":
    excel_filename = "HAL - ACC Roles - Consenting - Generated Roles.xlsx"
    create_sunburst_full_names(excel_filename)
